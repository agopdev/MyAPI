...
class MyApi
{
    ...
    private function __construct($routesController)
    {
        ...
        define("URL_CONTROLLERS", self::$root . "/src/routes/bases");
    ...
    {
        $requested_uri = self::$uri[1];
        $controller = URL_CONTROLLERS;

        if (array_key_exists($requested_uri, self::$routesController)) {
            $controllerName = self::$routesController[$requested_uri];
    ...
    private static function callRoute($controller)
    {
        ...
        $requested_uri = self::$uri[1];
        $controller = self::$routesController[$requested_uri];
        $routes = constant("$controller::routes");
        $sliced_uri = array_slice(self::$uri, 1);
        $requested_uri = "/" . implode("/", $sliced_uri);
        $requested_uri =
            substr($requested_uri, -1) === "/"
                ? rtrim($requested_uri, "/")
                : $requested_uri;
